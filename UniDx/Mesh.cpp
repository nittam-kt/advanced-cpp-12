...
void SubMesh::createVertexBuffer(void* data)
{
    ...
    // 頂点バッファの作成
    D3DManager::instance->GetDevice()->CreateBuffer(&vbDesc, &initData, &vertexBuffer);
}
...
void SubMesh::createIndexBuffer()
{
    ...
    // 頂点バッファの作成
    D3DManager::instance->GetDevice()->CreateBuffer(&vbDesc, &initData, &indexBuffer);
}
...
void SubMesh::Render() const
{
    ...
    UINT offset = 0;
    D3DManager::instance->GetContext()->IASetVertexBuffers(0, 1, vertexBuffer.GetAddressOf(), &stride, &offset);

    // プロミティブ・トポロジーをセット
    D3DManager::instance->GetContext()->IASetPrimitiveTopology(topology);

    // GPUへの描画命令発行
    if (indices.size() > 0 && indexBuffer)
    {
        // インデックスバッファを使う場合
        D3DManager::instance->GetContext()->IASetIndexBuffer(indexBuffer.Get(), DXGI_FORMAT_R32_UINT, 0);
        D3DManager::instance->GetContext()->DrawIndexed(static_cast<UINT>(indices.size()), 0, 0);
    }
    else
    {
        // 頂点データのみ場合
        D3DManager::instance->GetContext()->Draw(static_cast<UINT>(positions.size()), 0);
    }
}
