...
void LightManager::updateLightCBuffer()
{
    ...
    if (capacity_ < gpuLights_.size() || lightBuf_ == nullptr)
    {
        ...
        lightBuf_.Reset();
        D3DManager::instance->GetDevice()->CreateBuffer(&bd, nullptr, lightBuf_.GetAddressOf());
        ...
        D3DManager::instance->GetDevice()->CreateShaderResourceView(lightBuf_.Get(), &sd, lightSRV_.GetAddressOf());
    }
    ...
    if (!gpuLights_.empty())
    {
        ...
        D3DManager::instance->GetContext()->Map(lightBuf_.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &ms);
        memcpy(ms.pData, gpuLights_.data(), sizeof(GPULight) * gpuLights_.size());
        D3DManager::instance->GetContext()->Unmap(lightBuf_.Get(), 0);
    }
    /*
    // --- LightCount CB 更新 ---
...
*/
    ...
    D3DManager::instance->GetContext()->PSSetShaderResources(UNIDX_PS_SLOT_LIGHTS, 1, &srv);
}
